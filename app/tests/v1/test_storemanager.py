...
class TestStoreApp(unittest.TestCase):
    ...
    def setUp(self):
        ...
        '''This shall be appended to the specific url'''
        ...
        self.app.post(
            '{}users/registration'.format(self.base_url), 
    ...
    def test_whether_returns_status_code_on_products_query(self):
        self.assertEqual(self.app.get('{}products'.format(self.base_url), headers = dict(Authorization="Bearer " + self.auth_token), content_type='application/json').status_code, 200)
    
    def test_returns_message_if_no_products(self):
        response = self.app.get('{}products'.format(self.base_url), headers = dict(Authorization = "Bearer " + self.auth_token), content_type='application/json')
        assert response.status_code == 200
    
    """Returns 404 if the url is malformed and does not fetch data"""
    def test_malformed_url_on_products_query(self):
        response = self.app.get('{}produc'.format(self.base_url), headers = dict(Authorization="Bearer " + self.auth_token))
        self.assertEqual(response.status_code, 404)

    def test_adds_a_new_product(self):
        response = self.app.post('{}products'.format(self.base_url),headers = dict(Authorization="Bearer " + self.auth_token), data = json.dumps(self.sample_data), content_type='application/json')
        self.assertEqual(response.status_code,201)

    def test_malformed_post_one_product_url(self):
        response = self.app.post('{}products//'.format(self.base_url), data = json.dumps(self.sample_data), content_type='application/json')
        self.assertEqual(response.status_code,404)
    
    '''Test whether the api fetches a product successfull'''
    def test_get_a_product_by_id(self):
        response = self.app.get('{}products/1'.format(self.base_url),headers = dict(Authorization="Bearer " + self.auth_token), content_type='application/json')
        self.assertEqual(response.status_code,200)

    def test_malformed_url_on_a_given_product_query_by_id(self):
        response = self.app.get('{}products/1/wsd'.format(self.base_url))
        self.assertEqual(response.status_code,404)
    
    '''Returns a message if the id is ou of bounds'''
    def test_gives_error_feedback_if_product_id_out_of_bounds(self):
        response = self.app.get('{}products/0'.format(self.base_url),headers = dict(Authorization="Bearer " + self.auth_token),content_type='application/json')
    ...
    def test_adds_a_new_sale_order_successfully(self):
        ...
        response = self.app.post(
            '{}sales'.format(self.base_url),headers = dict(Authorization="Bearer " + self.auth_token), 
    ...
    def test_return_all_sales_orders(self):
        self.assertEqual(self.app.get('{}sales'.format(self.base_url),headers = dict(Authorization="Bearer " + self.auth_token), content_type='application/json').status_code, 200)
    
    '''Test whether a malformed url gives an error'''
    def test_malformed_url_for_get_all_products(self):
        response = self.app.get('{}saleswsd'.format(self.base_url))
        self.assertEqual(response.status_code,404)

    '''Test to see whether returns a specific sales order'''
    def test_gets_one_sales_order_by_id(self):
        response = self.app.get('{}sales/1'.format(self.base_url), headers = dict(Authorization="Bearer " + self.auth_token), content_type='application/json')
        self.assertEqual(response.status_code,200)
    
    """Test user registration"""
    def test_adds_a_new_user(self):
        response = self.app.post('{}users/registration'.format(self.base_url), data = json.dumps(self.sample_user), content_type='application/json')
        self.assertEqual(response.status_code, 201)

    #def test_user_login(self):
        #self.assertEqual(self.app.post('{}users/login'.format(self.base_url)).status_code, 200)